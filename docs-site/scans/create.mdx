...
active scan can also move to `paused`. Stopped and paused scans require the manual
**Resume** action. Failed scans do too by default; if **Auto-resume failed scans** is
enabled in Settings, the backend returns each scan to `pending` after it has remained
failed for about one second, without a browser action. Reaching `jobLimit` also stops a
scan after already-started jobs finish; raise or remove the limit before resuming it.
